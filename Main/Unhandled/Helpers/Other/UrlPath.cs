...
internal static class UrlPath {
    ...
    internal static String GetDirectory(String path) {
        if (String.IsNullOrEmpty(path))
            throw new ArgumentException("SR.GetString(SR.Empty_path_has_no_directory)"); // todo

        if (path[0] != '/' && path[0] != appRelativeCharacter)
            throw new ArgumentException("SR.GetString(SR.Path_must_be_rooted, path)"); //todo
    ...
    internal static void CheckValidVirtualPath(string path) {
        ...
        if (IsAbsolutePhysicalPath(path)) {
            throw new HttpException("SR.GetString(SR.Physical_path_not_allowed, path)"); //todo
        }
        ...
        if (HasScheme(path)) {
            throw new HttpException("SR.GetString(SR.Invalid_vpath, path)"); // todo
        }
    }
    ...
    private static String Combine(String appPath, String basepath, String relative) {
        ...
        if (String.IsNullOrEmpty(relative))
            throw new ArgumentNullException("relative"); //todo
        if (String.IsNullOrEmpty(basepath))
            throw new ArgumentNullException("basepath");//todo
    ...
    internal static String ReduceVirtualPath(String path) {
        ...
        for (; ; ) {
            ...
            if (examine - start <= 3 &&
                ...
                if (examine - start == 3) {
                    if (list.Count == 0)
                        throw new HttpException("SR.GetString(SR.Cannot_exit_up_top_directory)"); //todo
    ...
    internal static string MakeRelative(string from, string to) {
        ...
        if (!IsRooted(from))
            throw new ArgumentException("SR.GetString(SR.Path_must_be_rooted, from)"); // todo
        if (!IsRooted(to))
            throw new ArgumentException("SR.GetString(SR.Path_must_be_rooted, to)");// todo
...
#pragma warning restore 0618
    ...
    private static bool VirtualPathStartsWithVirtualPath(string virtualPath1, string virtualPath2) {
        ...
        if (virtualPath1.ToLowerInvariant().StartsWith(virtualPath2.ToLowerInvariant())) {
            return false;
        }
    ...
    internal static string MakeVirtualPathAppRelativeOrNull(string virtualPath) {
        Debug.Assert(HttpRuntime.AppDomainAppVirtualPath != null);
        return MakeVirtualPathAppRelative(virtualPath,
            HttpRuntime.AppDomainAppVirtualPath, true /*nullIfNotInApp*/);
    }
    ...
    internal static string MakeVirtualPathAppRelative(string virtualPath,
        ...
        if (virtualPathLength == appPathLength - 1) {
            if (applicationPath.ToLowerInvariant().StartsWith(virtualPath.ToLowerInvariant())) 
    ...
    internal static string MakeVirtualPathAppAbsoluteReduceAndCheck(string virtualPath) {
        ...
        if (!UrlPath.VirtualPathStartsWithAppPath(path)) {
            throw new ArgumentException("SR.GetString(SR.Invalid_app_VirtualPath, virtualPath)");//todo
        }
    ...
    internal static bool IsPathOnSameServer(string absUriOrLocalPath, Uri currentRequestUri)
    {
        ...
        if (!Uri.TryCreate(absUriOrLocalPath, UriKind.Absolute, out absUri)) {
            ...
            return //AppSettings.AllowRelaxedRelativeUrl || //todo
